...
import com.owen233666.util.ExposureReflection;
...
public class ExposurePhotoUtil {
    ...
    private static void ensureExposureLoaded() {
        ...
        try {
            ...
            exposureClientClass = ExposureReflection.loadClass(EXPOSURE_CLIENT_CLASS);
            renderedExposuresClass = ExposureReflection.loadClass(RENDERED_EXPOSURES_CLASS);
            imageRendererClass = ExposureReflection.loadClass(IMAGE_RENDERER_CLASS);
            renderableImageClass = ExposureReflection.loadClass(RENDERABLE_IMAGE_CLASS);
            ...
            // Either Exposure is absent or its API differs from what we reflect against. Drop the
            // integration, but leave every handle null so the guarded call sites cannot NPE.
            exposureAvailable = false;
            photographItemClass = null;
            exposureClientClass = null;
            renderedExposuresClass = null;
            imageRendererClass = null;
            renderableImageClass = null;
            getFrameMethod = null;
            renderedExposuresMethod = null;
            getOrCreateMethod = null;
            isEmptyMethod = null;
            imageRendererMethod = null;
            renderMethod = null;
            ExposureReflection.warnUnavailableOnce("ExposurePhotoUtil", IMAGE_RENDERER_CLASS + "#render", t);
        }
        // Settled only now, so a half-failed probe is retried rather than latched off forever.
    ...
    public static void renderPhotoOrPainting(@Nullable Object exposureImage,
        ...
        if (exposureImage != null) {
            // NOTE: v1/v2 are swapped relative to the painting path on purpose. Exposure's image
            // renderer uses a top-left origin while Minecraft block textures use bottom-left, so
            // the vertical range has to be flipped or photographs render upside down.
