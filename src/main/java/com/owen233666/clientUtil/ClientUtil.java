...
public class ClientUtil {
    ...
    public static void renderTexture(ResourceLocation resourceLocation, PoseStack poseStack, MultiBufferSource multiBufferSource,
        ...
        emitVertex(vertexConsumer, matrix4f, x1, y2, u1, v2, packedLight, r, g, b, a);
        emitVertex(vertexConsumer, matrix4f, x2, y2, u2, v2, packedLight, r, g, b, a);
        emitVertex(vertexConsumer, matrix4f, x2, y1, u2, v1, packedLight, r, g, b, a);
        emitVertex(vertexConsumer, matrix4f, x1, y1, u1, v1, packedLight, r, g, b, a);
    }

    /**
     * Emits one quad vertex at z = 0.
     *
     * <p>Newer versions replaced the {@code vertex(...)/color(...)/uv(...)/uv2(...)/endVertex()}
     * chain with {@code addVertex(...)/setColor(...)/setUv(...)/setLight(...)}, so both spellings
     * are kept here and selected by version.
     */
    private static void emitVertex(VertexConsumer vertexConsumer, Matrix4f matrix4f,
                                   float x, float y, float u, float v, int packedLight,
                                   int r, int g, int b, int a) {
        //#if MC >= 12005
        vertexConsumer.addVertex(matrix4f, x, y, 0.0F).setColor(r, g, b, a).setUv(u, v).setLight(packedLight);
        //#else
        //$$ vertexConsumer.vertex(matrix4f, x, y, 0.0F).color(r, g, b, a).uv(u, v).uv2(packedLight).endVertex();
        //#endif
    }
}